...
- variety: distinguishing characteristics beyond the base product type (flavor, style, preparation)
- size: normalized format with space ("8oz" -> "8 oz", "1.5LB" -> "1.5 lb")
- isStoreGeneric: true for store brands — Publix, Great Value (Walmart), Good & Gather (Target), Market Pantry (Target), Member's Mark (Sam's Club), 365 (Whole Foods), Up & Up (Target)
...
- Same UPC or same brand + same type + same size = 0.95+
- Same type + similar size + both store generics = 0.85+
- Same type + different size = 0.6-0.7 (note size difference in reason)
- Different type entirely = do not include
- Store generics from different stores are interchangeable (Publix brand ≈ Great Value ≈ Good & Gather)
...
export class ClaudeProvider implements AIProvider {
  ...
  async parseReceiptText(text: string, context: ReceiptParseContext) {
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 4096,
      messages: [
        { role: "user", content: buildReceiptTextPrompt(text, context) },
      ],
    });
  ...
  async extractProductAttributes(rawNames: string[]) {
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 4096,
      messages: [
        { role: "user", content: buildExtractAttributesPrompt(rawNames) },
      ],
    });
  ...
  async matchProductsAcrossStores(
    ...
    const response = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: buildCrossStoreMatchPrompt(sourceProduct, candidates),
        },
      ],
    });
